...
workflow Mutect2 {
    ...
    String output_basename = basename(basename(tumor_reads, ".bam"),".cram")
    ...
    if (run_funcotator_or_default) {
        ...
        call Funcotate {
            input:
                ...
                output_format = select_first([funco_output_format, funco_default_output_format]),
                ...
                transcript_selection_list = if defined(funco_transcript_selection_list) then [select_first([funco_transcript_selection_list])] else [],
...
task CramToBam {
    ...
    Int machine_mem = if defined(mem) then select_first([mem]) * 1000 else 6000
...
task Funcotate {
     input {
       ...
       Boolean compress
       Boolean use_gnomad
       File? data_sources_tar_gz   # supply local/https path for miniwdl
       String? control_id
       String? case_id
       String? sequencing_center
       String? sequence_source
       ...
       Array[File]? annotation_defaults
     ...
     Boolean do_filter = select_first([filter_funcotations, false]) String filter_funcotations_args = if do_filter then " --remove-filtered-variants " else ""
