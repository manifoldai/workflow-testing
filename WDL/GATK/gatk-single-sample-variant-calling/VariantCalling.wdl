...
workflow VariantCalling {
  ...
  String gatk_docker = if cloud_provider == "gcp" then gatk_docker_gcp else (if cloud_provider == "aws" then gatk_docker_aws else gatk_docker_azure)
  ...
  String gatk_1_3_docker_aws = "broadinstitute/gatk:1.3.0"
  String gatk_1_3_docker = if cloud_provider == "gcp" then gatk_1_3_docker_gcp else (if cloud_provider == "aws" then gatk_1_3_docker_aws else gatk_1_3_docker_azure)
  ...
  String picard_python_docker_aws = "broadinstitute/picard:latest"
  String picard_python_docker = if cloud_provider == "gcp" then picard_python_docker_gcp else (if cloud_provider == "aws" then picard_python_docker_aws else picard_python_docker_azure)
  ...
  String picard_cloud_docker_aws = "broadinstitute/picard:latest"
  String picard_cloud_docker = if cloud_provider == "gcp" then picard_cloud_docker_gcp else (if cloud_provider == "aws" then picard_cloud_docker_aws else picard_cloud_docker_azure)


  # make sure either gcp, azure, or aws is supplied as cloud_provider input
  if ((cloud_provider != "gcp") && (cloud_provider != "azure") && (cloud_provider != "aws")) {
    call Utils.ErrorWithMessage as ErrorMessageIncorrectInput {
      input:
        message = "cloud_provider must be supplied with either 'gcp', 'azure', or 'aws'."
    }
  }
