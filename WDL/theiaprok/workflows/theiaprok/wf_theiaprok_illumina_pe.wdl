...
workflow theiaprok_illumina_pe {
  ...
  input {
    ...
    File? gamma_db_override
    File? kmerfinder_db_override
    # Merlin database file overrides
    File? merlin_snippy_reference_afumigatus
    File? merlin_snippy_reference_cryptoneo
    File? qc_check_table
  }
  ...
  if (defined(read1_lane2)) {
    call concatenate_lanes_workflow.concatenate_illumina_lanes {
      input:
        ...
        cat_lanes_docker_image = cat_lanes_docker_image,
        version_capture_docker_image = version_capture_docker_image
    }
  }
  if (! skip_screen) {
  ...
  if (select_first([raw_check_reads.read_screen, ""]) == "PASS" || skip_screen) {
    call read_qc.read_QC_trim_pe as read_QC_trim {
      input:
        ...
        workflow_series = "theiaprok",
        fastqc_docker_image = fastqc_docker_image,
        fastq_scan_docker_image = fastq_scan_docker_image,
        trimmomatic_docker_image = trimmomatic_docker_image,
        fastp_docker_image = fastp_docker_image,
        bbduk_docker_image = bbduk_docker_image,
        kraken2_docker_image = kraken2_docker_image,
        ncbi_scrub_docker_image = ncbi_scrub_docker_image,
        midas_docker_image = midas_docker_image,
        readlength_docker_image = readlength_docker_image
    }
    if (! skip_screen) {
    ...
    if (select_first([clean_check_reads.read_screen, ""]) == "PASS" || skip_screen) {
      call digger_denovo.digger_denovo {
        input:
          ...
          read1 = read_QC_trim.read1_clean,
          read2 = read_QC_trim.read2_clean,
          spades_docker = spades_docker_image,
          skesa_docker = skesa_docker_image,
          megahit_docker = megahit_docker_image,
          bwa_docker = bwa_docker_image,
          pilon_docker = pilon_docker_image,
          filter_contigs_docker = filter_contigs_docker_image
      }
      ...
      if (perform_characterization) {
        ...
        call merlin_magic_workflow.merlin_magic {
          input:
            ...
            read1 = read_QC_trim.read1_clean,
            read2 = read_QC_trim.read2_clean,
            abricate_abaum_docker_image = merlin_abricate_abaum_docker_image,
            abricate_vibrio_docker_image = merlin_abricate_vibrio_docker_image,
            agrvate_docker_image = merlin_agrvate_docker_image,
            amr_search_docker_image = merlin_amr_search_docker_image,
            cauris_cladetyper_docker_image = merlin_cauris_cladetyper_docker_image,
            clockwork_docker_image = merlin_clockwork_docker_image,
            ectyper_docker_image = merlin_ectyper_docker_image,
            emmtyper_docker_image = merlin_emmtyper_docker_image,
            emmtypingtool_docker_image = merlin_emmtypingtool_docker_image,
            genotyphi_docker_image = merlin_genotyphi_docker_image,
            hicap_docker_image = merlin_hicap_docker_image,
            kaptive_docker_image = merlin_kaptive_docker_image,
            kleborate_docker_image = merlin_kleborate_docker_image,
            legsta_docker_image = merlin_legsta_docker_image,
            lissero_docker_image = merlin_lissero_docker_image,
            meningotype_docker_image = merlin_meningotype_docker_image,
            ngmaster_docker_image = merlin_ngmaster_docker_image,
            pasty_docker_image = merlin_pasty_docker_image,
            pbptyper_docker_image = merlin_pbptyper_docker_image,
            poppunk_docker_image = merlin_poppunk_docker_image,
            seqsero2s_docker_image = merlin_seqsero2s_docker_image,
            seroba_docker_image = merlin_seroba_docker_image,
            serotypefinder_docker_image = merlin_serotypefinder_docker_image,
            shigatyper_docker_image = merlin_shigatyper_docker_image,
            shigeifinder_docker_image = merlin_shigeifinder_docker_image,
            sistr_docker_image = merlin_sistr_docker_image,
            snippy_gene_query_docker_image = merlin_snippy_gene_query_docker_image,
            snippy_variants_docker_image = merlin_snippy_variants_docker_image,
            sonneityping_docker_image = merlin_sonneityping_docker_image,
            spatyper_docker_image = merlin_spatyper_docker_image,
            srst2_docker_image = merlin_srst2_docker_image,
            staphopia_sccmec_docker_image = merlin_staphopia_sccmec_docker_image,
            tbprofiler_docker_image = merlin_tbprofiler_docker_image,
            tbp_parser_docker_image = merlin_tbp_parser_docker_image,
            vibecheck_docker_image = merlin_vibecheck_docker_image,
            virulencefinder_docker_image = merlin_virulencefinder_docker_image,
            stxtyper_docker_image = merlin_stxtyper_docker_image,
            snippy_reference_afumigatus = select_first([merlin_snippy_reference_afumigatus, "gs://theiagen-public-resources-rp/reference_data/eukaryotic/aspergillus/Aspergillus_fumigatus_GCF_000002655.1_ASM265v1_genomic.gbff"]),
            snippy_reference_cryptoneo = select_first([merlin_snippy_reference_cryptoneo, "gs://theiagen-public-resources-rp/reference_data/eukaryotic/cryptococcus/Cryptococcus_neoformans_GCF_000091045.1_ASM9104v1_genomic.gbff"])
        }
          ...
          call export_taxon_table_task.export_taxon_table {
            input:
              ...
              columns_to_export = {
                ...
                "city": city,
                "clockwork_decontaminated_read1": merlin_magic.clockwork_cleaned_read1,
                "clockwork_decontaminated_read2": merlin_magic.clockwork_cleaned_read2,
                ...
                "fastq_scan_num_reads_clean_pairs": read_QC_trim.fastq_scan_clean_pairs,
                "fastq_scan_num_reads_clean1": read_QC_trim.fastq_scan_clean1,
                "fastq_scan_num_reads_clean2": read_QC_trim.fastq_scan_clean2,
                "fastq_scan_num_reads_raw_pairs": read_QC_trim.fastq_scan_raw_pairs,
                "fastq_scan_num_reads_raw1": read_QC_trim.fastq_scan_raw1,
                "fastq_scan_num_reads_raw2": read_QC_trim.fastq_scan_raw2,
                ...
                "fastqc_clean2_html": read_QC_trim.fastqc_clean2_html,
                "fastqc_docker": read_QC_trim.fastqc_docker,
                "fastqc_num_reads_clean_pairs": read_QC_trim.fastqc_clean_pairs,
                "fastqc_num_reads_clean1": read_QC_trim.fastqc_clean1,
                "fastqc_num_reads_clean2": read_QC_trim.fastqc_clean2,
                "fastqc_num_reads_raw_pairs": read_QC_trim.fastqc_raw_pairs,
                "fastqc_num_reads_raw1": read_QC_trim.fastqc_raw1,
                "fastqc_num_reads_raw2": read_QC_trim.fastqc_raw2,
                "fastqc_raw1_html": read_QC_trim.fastqc_raw1_html,
                "fastqc_raw2_html": read_QC_trim.fastqc_raw2_html,
                ...
                "read2_concatenated": concatenate_illumina_lanes.read2_concatenated,
                "read2": select_first([concatenate_illumina_lanes.read2_concatenated, read2]),
