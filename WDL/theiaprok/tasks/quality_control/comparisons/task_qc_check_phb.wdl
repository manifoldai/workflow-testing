...
task qc_check_phb {
  ...
  command <<<
    ...
    # "_min" and "_max" suffixes are allowed in qc_check_table columns to override operator
    qc_check_criteria = {
      ...
      "num_reads_clean1": {"type": int, "operator": operator.ge},
      "num_reads_clean2": {"type": int, "operator": operator.ge},
    ...
    def compare(qc_note, variable_name, input_value, obs_operator, standard):
      ...
      if input_value is not None and not pd.isnull(input_value): 
        ...
        if obs_operator(input_value, standard):
          print(f"DEBUG: {variable_name} ({input_value}) was {op2str[obs_operator]} the threshold of {standard}")
        else:
    ...
    with open("~{qc_check_input_json}", "r") as f:
      ...
      # check if using normal mapping
      if isinstance(qc_check_inputs_dirty, dict):
        qc_check_inputs = {k.lower(): v for k, v in qc_check_inputs_dirty.items() if v is not None}
      # check if using cromwell mapping
      else:
        qc_check_inputs = {}
        for wdl_dict in qc_check_inputs_dirty:
          obs_val = wdl_dict["right"]
          if obs_val is not None:
            qc_check_inputs[wdl_dict["left"].lower()] = obs_val
